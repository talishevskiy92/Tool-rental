		...
		if(Edit1->Text.ToInt() <= DBText4->Caption.ToInt() && Edit1->Text.ToInt() != 0 && DateTimePicker1->Date != DateTimePicker2->Date){
		   ...
		   //	DM->Sklad_I->FieldByName("Kilkist")->AsString = DBText4->Caption - Edit1->Text;
			...
			//DM->Kohuk->Append();
				...
				MessageDlg("Некоректна дата заврешення оренди",mtInformation,TMsgDlgButtons()<<mbOK,0);
						}
		else{
			MessageDlg("Дані не збережені",mtInformation,TMsgDlgButtons()<<mbOK,0);
			MessageDlg("Недостатння кількість в наявності",mtInformation,TMsgDlgButtons()<<mbOK,0);
	...
	if(DBLookupComboBox1->Text==""){
		...
		if(Edit2->Text.ToInt() <= DBText3->Caption.ToInt() && Edit2->Text.ToInt() != 0 && DateTimePicker5->Time != DateTimePicker6->Time){
		   ...
		   //	MessageDlg("Дані успішно збережені",mtInformation,TMsgDlgButtons()<<mbOK,0);
			...
			MessageDlg("Дані не збережені",mtInformation,TMsgDlgButtons()<<mbOK,0);
			MessageDlg("Некоректний час заврешення оренди",mtInformation,TMsgDlgButtons()<<mbOK,0);
		}
		else{
            MessageDlg("Дані не збережені",mtInformation,TMsgDlgButtons()<<mbOK,0);
			MessageDlg("Недостатння кількість в наявності",mtInformation,TMsgDlgButtons()<<mbOK,0);
